...
export default function useBarDragManagement (
  ...
  const moveBarByMinutes = (bar: GanttBarObject, minutes: number, direction: "left" | "right") => {
    switch (direction) {
      case "left":
        bar[barStart.value] = toDayjs(bar).subtract(minutes, "minutes").format(dateFormat.value)
        bar[barEnd.value] = toDayjs(bar).subtract(minutes, "minutes").format(dateFormat.value)
        break
      case "right":
        bar[barStart.value] = toDayjs(bar).add(minutes, "minutes").format(dateFormat.value)
        bar[barEnd.value] = toDayjs(bar).add(minutes, "minutes").format(dateFormat.value)
    }
    fixOverlaps(bar)
  }
